...
def parse_config():
    ...
    parser.add_argument('--cfg', type=str,default='kitti.yaml', help='the config_file')
    ...
    parser.add_argument('--preloading', action='store_true', help='preload dataset into memory')
...
if not cfg.test:
    ...
    # init trainer
    trainer = pl.Trainer(gpus=-1,  # Number of GPUs used (int) or list of GPU nodes
                         accelerator='ddp',  # Distributed training
                         max_epochs=cfg.epoch,
                         resume_from_checkpoint=cfg.checkpoint,
                         callbacks=[checkpoint_callback],  # Add callback functions or a list of callback functions
